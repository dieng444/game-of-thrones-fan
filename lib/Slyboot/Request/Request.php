...
class Request
{
    ...
    public function __construct()
    {
        ...
        if ($this->http->getMethod()==='GET' && $this->http->getMethod()!=='POST') {
            ...
            $uri = str_replace(dirname($this->http->getScriptName()), '', $this->http->getRequestUri());
            $script = dirname(dirname($this->http->getScriptName()));
            $relativeUri = str_replace($script, '', $this->http->getRelativeRequestUri());
            $main_rel_uri = $this->http->getRelativeRequestUri();
                        ...
                        if (preg_match($pattern, $relativeUri, $match)) {
                            ...
                            if (sizeof($route->getQueryString()) > 0) {
                                ...
                                foreach ($route->getQueryString() as $k => $val) {
                                    //var_dump($tab_uri_query_s_clean);
