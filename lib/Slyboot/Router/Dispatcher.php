    ...
    public function dispatch()
    {
        ...
        ************************************************/
        ...
        if (sizeof($collections) > 0) {
                ...
                foreach ($collection->getRoutes() as $route) {
                    ...
                    if (preg_match($pattern, $relativeUri, $match)) {
                        ...
                        if (substr($route->getFullPath(), $fullPathPos, 1)===$script) {
                            throw new Exception(" Error 404 [] remove the last \"/\" behind the route path in the routing file");
                        ...
                        ****************************************************/
                            if (substr($uri, $pos, 1)!==$script) {
                                throw new Exception("Error 404 the current route does not exist.");
                                die;
                            } else {
                                        ...
                                        if (method_exists($controller, $action)) {
                                            ...
                                            ************************************************************/
                                            ...
                                            if (sizeof($route->getQueryString())!==(sizeof($parameters))) {
                                                echo "Missed parameters, count of parameters not corresponding";
                                                die;
                                            } else {
                                                /**
                                                ...
                                                */
                                                ...
                                                foreach ($route->getQueryString() as $k => $v) {
                                                    if ($route->getQueryString()[$k]!==$tabParams[$k]) {
                                                        ...
                                                        throw new Exception(
                                                            "Param {$v} does not a parameter of
                                            ...
                                            throw new Exception("Unknow method exception[] Method {$action}
                                                                 ...
                                                                 is wrong in the routing file. Learn more about route definition here
                                                                  www.slyboot.com/doc/routing");
                                        }
                                    }
                                } else {
                                    throw new Exception("Error 404 count of route parameter, does not match");
                         ...
                         *******************************************************/
                        } elseif (!(sizeof($route->getQueryString()) > 0)) {
                            //var_dump($pos);
                            //var_dump(strlen($uri));die;
                            if (strlen($uri)!==$pos) {
                                throw new Exception("Error 404 the current route does'nt exit");
                                die;
                            } else {
                                $controllerClass = $route->getVendor().'\\'.
                                                  $route->getPackage().
            ...
            throw new Exception("Error 404 no route found");
        }
    }
}
