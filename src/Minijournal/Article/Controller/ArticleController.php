...
class ArticleController extends Controller
{
    ...
    public function listAction()
    {
        ...
        return $this->render(
            'Minijournal::Article::Default::article-list.html.twig',
            array('articles' => $data)
        );
    }
    public function detailAction($id)
    {
        ...
        return $this->render(
            'Minijournal::Article::Default::article-detail.html.twig',
                ...
                if ($this->request->isMethod('post')) {
                    ...
                    if ($form->isValid()) {
                        ...
                        if ($article->getId()!==null) {
                            $this->redirect("/Slyboot-1.1.0/article/detail/{$article->getId()}");
                        } else {
                            $this->redirect("/Slyboot-1.1.0/article/detail/{$id}");
                        }
                    } else {
                        return $this->render(
                            'Minijournal::Article::Default::article-form.html.twig',
                            array('form' => $form)
                        );
                    }
                } else {
    ...
    public function removeAction($id)
    {
        if ($this->user->isConnected()) {
            if ($this->user->isGrantRole('ROLE_EDITOR')) {
                ...
                /****Suppression des images liées à l'article encours de suppression*****/
                if (sizeof($images) > 0) {
                ...
                return $this->redirect("/Slyboot-1.1.0/article/list");
            } else {
                throw new Exception("Access Denied : You don't have permission to execute this action");
            }
        } else {
